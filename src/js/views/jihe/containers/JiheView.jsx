...
export default class extends React.Component {
    ...
    getItemViews(){
      return _.map(this.state.datas,function(item){
        let key = item.aid ? item.aid : _.uniqueId();
        return (
          <a href={item.link} key={key} className='swiper'>
            <img src={item.cover} alt={item.title} className="swipeImage"/>
          <h3>{item.title}</h3>
        </a>
      )
      })
    ...
    getListItemViews(data){
      return _.map(data,function(item){
        let key = item.aid ? item.aid : _.uniqueId();
        return (
              <a href={item.link} key={key} className="list">
    ...
    onChangeTab(tabIndex){
      ...
      //根据tab切换下边显示的内容
      this.refs.listSwipe.slide(tabIndex);
    ...
    render() {
        ...
        let listViewOne  = this.getListItemViews(this.state.datas);
        let listViewTwo  = this.getListItemViews(this.state.listDatas);
        let _this = this;
        ...
        return (
            <App active="youxi">
                <div>
                  {/* banner图 */}
                  <ReactSwipe swipeOptions={{continuous:true,auto:5000}}>
                      {this.getItemViews()}
                  </ReactSwipe>
                  {/* 导航菜单 */}
                  <ul className="nav">
                    <li onClick={this.onChangeTab.bind(this,0)} className={this.tabClass(0)}>杭州</li>
                    <li onClick={this.onChangeTab.bind(this,1)} className={this.tabClass(1)}>莫干山</li>
                    <li onClick={this.onChangeTab.bind(this,2)} className={this.tabClass(2)}>苏浙皖</li>
                    <li onClick={this.onChangeTab.bind(this,3)} className={this.tabClass(3)}>云闽川</li>
                    <li onClick={this.onChangeTab.bind(this,4)} className={this.tabClass(4)}>其他</li>
                  </ul>
                  {/* 主要内容部分 */}
                  <ReactSwipe ref="listSwipe" swipeOptions={{continuous:false,callback:function(index){
                      _this.setState({
                        activeIndex:index
                      })
                    }}}>
                    <div style={{height: _this.state.activeIndex == 0 ? 'auto' : SWIPE_MIN_HEIGHT, overflow: 'hidden'}}>
                      {listViewOne}
                    </div>
                    <div style={{height: _this.state.activeIndex == 1 ? 'auto' : SWIPE_MIN_HEIGHT, overflow: 'hidden'}}>
                      {listViewTwo}
                    </div>
                    <div style={{height: _this.state.activeIndex == 2 ? 'auto' : SWIPE_MIN_HEIGHT, overflow: 'hidden'}}>
                      {listViewOne}
                    </div>
                    <div style={{height: _this.state.activeIndex == 3 ? 'auto' : SWIPE_MIN_HEIGHT, overflow: 'hidden'}}>
                      {listViewTwo}
                    </div>
                    <div style={{height: _this.state.activeIndex == 4 ? 'auto' : SWIPE_MIN_HEIGHT, overflow: 'hidden'}}>
                      {listViewOne}
                    </div>
                  </ReactSwipe>
                </div>
